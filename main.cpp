/*
 * SNAKE GAME:
 *   Made by Madularea Denisa, IESC, TI, 4LF322
 */
...
void drawControls() {
    char* move_message = (char*)"   Use the arrow keys to move";
...
float snake_color_red = 0.0f;
float snake_color_green = 1.0f;
float snake_color_blue = 0.0f;

void drawSnake() {
    // Loop over snake size and draw each part at its respective coordinates
    for (unsigned int a = 0; a < part_coords.size(); a++) {
        glLoadIdentity();
        glTranslatef(part_coords[a][0], part_coords[a][1], -40.0f);
        glColor3f(snake_color_red, snake_color_green, snake_color_blue);
...
void chooseSnakeColor() {
    std::cout << "Enter the red component (0.0 - 1.0) for the snake: ";
    std::cin >> snake_color_red;
    std::cout << "Enter the green component (0.0 - 1.0) for the snake: ";
    std::cin >> snake_color_green;
    std::cout << "Enter the blue component (0.0 - 1.0) for the snake: ";
    std::cin >> snake_color_blue;
}
